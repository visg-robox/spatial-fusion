...
"""Arguments
 ...
 """
def sparse_conv(tensor, binary_mask=None, filters=32, kernel_size=3, strides=1, l2_scale=0.0, rate = 1, scope=None):
    with tf.variable_scope(scope):
        ...
        features = tf.layers.conv2d(features, filters=filters, kernel_size=kernel_size, strides=(strides, strides),
                                                                    ...
                                                                    kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=l2_scale), dilation_rate=rate)
        ...
        feature = tf.multiply(features, norm)
        feature = layers.batch_norm(feature, activation_fn=nn_ops.relu)
        binary_mask = layers.max_pool2d(binary_mask, [3, 3], stride=1)

    return feature, binary_mask

# def sparse_conv_back(tensor, binary_mask=None, filters=32, kernel_size=3, strides=2, l2_scale=0.0):
#     if binary_mask == None:  # first layer has no binary mask
#         b, h, w, c = tensor.get_shape()
#         channels = tf.split(tensor, c, axis=3)
#         # assume that if one channel has no information, all channels have no information
