...
def eval_print_save(total_accuracy,method_name,logdir,classname,Forcerecord=True):
    ...
    #     MIOU=miou
        ...
        for i in range(classname.shape[0]):
            ...
            booksheet.write(1, i+3, format(per_class_accuracy[i],'.2%'))
            booksheet.write(4, i + 3,format(per_class_iou[i],'.2%'))
            booksheet.write(7, i + 3, format(total_accuracy[i,2]/np.sum(total_accuracy[:, 2]),'.2%'))
        booksheet.write(0,2,'mean')
        booksheet.write(1, 2 ,format(mean_accuracy,'.2%'))
        booksheet.write(4, 2  , format(miou,'.2%'))
