...
class Camera(Sensor):
    ...
    def set_PostProcessing(self,type):
    ...
    def get_unreal_transform(self):
        ...
        '''
        to_unreal_transform = Transform(Rotation(roll=-90, yaw=90), Scale(y=-1))
    ...
    def get_instrincs(self):
        k = numpy.identity(3)
        k[0, 2] = self.ImageSizeX / 2.0
        k[1, 2] = self.ImageSizeY / 2.0
        k[0, 0] = k[1, 1] =self.ImageSizeX / \
                            (2.0 * math.tan(self.FOV * math.pi / 360.0))
        return k
...
class PointCloud(SensorData):
    ...
    def __init__(self, frame_number, array, color_array=None,sem_array=None):
        ...
        self._sem_array=sem_array
        self._has_sem = sem_array is not None
    ...
    def save_to_disk(self, filename):
        ...
        if  self._has_colors:
            ...
            points_3d = numpy.concatenate(
                (self._array,self._sem_array), axis=1)
            ply = '\n'.join(['{:.2f} {:.2f} {:.2f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}'
                             .format(*p) for p in points_3d.tolist()])
        else:
...
class LidarMeasurement(SensorData):
    ...
    def apply_transform(self, transformation):
        """Modify the PointCloud instance transforming its points"""
        self.point_cloud.apply_transform(transformation)
