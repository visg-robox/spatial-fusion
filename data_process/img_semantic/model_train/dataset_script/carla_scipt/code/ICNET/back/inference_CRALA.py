...
IMG_PATH='/media/luo/project/project/semantic_segmentation/data/dataset/CARLA/CARLA_/PythonClient/Data/out_lidar_vedio_test/episode_19/Data/RGB'
...
def main():
    ...
    for camera_dir in camera_dir_list:
        ...
        for imgpath in pathlist:
            ...
            pred_ID,pred_P = sess.run([raw_output_ID[0],raw_output_P[0]],feed_dict={x: img})
            pred_ID=np.repeat(np.expand_dims(pred_ID,axis=2),3,axis=2)

            misc.imsave(imgpath.replace('RGB','Infer_sem'),np.uint8(pred_ID))
            #np.save(imgpath.replace('RGB','Infer_sem').replace('.png','.npy'),pred_P)
