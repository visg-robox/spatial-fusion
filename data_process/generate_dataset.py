...
if __name__ == "__main__":
    ...
    move_length = len(infer_feature_file)//5

    infer_feature_file_move = infer_feature_file[-move_length:]
    infer_label_file_move = infer_label_file[-move_length:]
    ...
    for item in infer_label_file_move:
        ...
        shutil.move(cur_infer_label_file, test_p_save_path + fname)

    for item in gt_file_move:
        cur_gt_file = item
        fpath, fname = os.path.split(cur_gt_file)
        shutil.move(cur_gt_file, test_gt_save_path + fname)
