...
nvcc -std=c++11 -ccbin=/usr/bin/g++ -c -o deform_conv.cu.o deform_conv.cu.cc -I $TF_INC -I $NSYNC_INC -I /usr/local -D\
          GOOGLE_CUDA=1 -x cu -Xcompiler -fPIC -L $CUDA_HOME/lib64/ --expt-relaxed-constexpr -arch=$ARCH -D_GLIBCXX_USE_CXX11_ABI=0
## if you install tf using already-built binary, or gcc version 4.x, uncomment the three lines below
