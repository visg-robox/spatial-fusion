...
class Network(object):
    ...
    @layer
    def element_multi(self,input,name):
        return tf.multiply(input[0],input[1],name=name)

    @layer
    def resize_nearest(self,input,shape,name):
        resized=tf.image.resize_images(input,shape,method=1)
        return resized

    @layer
    def tile(self,input,multip,name):
        return tf.tile(input,multiples=multip,name=name)
    ...
    @layer
    def neg(self, input, name):
        return tf.negative(input, name=name)
