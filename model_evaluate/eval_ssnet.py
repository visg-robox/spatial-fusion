...
def eval_spnet_balance_multi_process(all_path,
    ...
    eval_print_save(total_accuracy_rnn, evaluate_name, log_dir, points_num=(len(test_keys_list) // TEST_BATCH_SIZE) * TEST_BATCH_SIZE)
    return test_loss_all
...
def do(all_path, model, model_path, save_path):
    loss = eval_spnet_balance_multi_process(all_path, model, model_path, time_step=common.time_step, log_dir=save_path, ignore_list=common.ignore_list)
    return loss
...
def eval_spnet_multi_process(model_path, scene_name="Record006/"):
    ...
    eval_print_save(total_matrix, model_path.split('/')[-1].split('.')[0], os.path.dirname(save_path),  points_num = points_num)
'''
