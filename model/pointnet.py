...
class Pointnet(nn.Module):
    def __init__(self, capacity = CAPACITY, class_num = common.class_num, input_dim = INPUT_FEATURE_DIM, gpu = True):
        ...
        self.conv3_1 = nn.Conv2d(capacity * 17, capacity * 8, 1)
...
class input_transform_net(nn.Module):
    def __init__(self, capacity = CAPACITY, k = 3, gpu = True):
        assert (k == 3)
        ...
        self.conv3 = nn.Conv2d(capacity*2, capacity*16, 1)
        ...
        init_weight = torch.Tensor(np.zeros([3 * k , capacity *4, 1, 1], dtype= np.float32))
    ...
    def forward(self, pointcloud_xyz):
        num_point = pointcloud_xyz.size(2)
        batch_size = pointcloud_xyz.size(0)
        k_dim = pointcloud_xyz.size(1)
        ...
        pointcloud_xyz = pointcloud_xyz.permute(0,2,3,1) #shape(B, num, 1, k)
        out_xyz = torch.matmul(pointcloud_xyz, transform_mat)#shape(B, num, 1, 3)
        out_xyz = out_xyz.permute(0, 3, 1, 2)
        out_xyz.squeeze(dim = 2)
        return out_xyz
